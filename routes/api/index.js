...
router
  .get("/comment", async function(req, res, next) {
  ...
  .get("/note/:id/comment", async (req, res) => {
    const { id } = req.params;
    const comment = await Comments.findOne({ parentId: id }, { __v: 0 });
    if (!comment) return res.status(404).json({ message: "comment not found" });
    return res.status(200).json(comment);
  })
  ...
  .post("/comment", async (req, res) => {
    ...
    console.log(err);
    if (err)
      return res.status(500).json({ message: "save error", detail: err });
  ...
  .get("/note/chepter/:id/comments", async (req, res) => {
    const { id } = req.params;
    const comments = Comments.find({ chapterId: id });
    if (!comments) return res.status(404).json({ message: "not found" });
    return res.status(200).json(comments);
  });
